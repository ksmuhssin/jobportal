...
class ModelMegaheaderMegaheader extends Model {
	public function addMegaheader($data) { //print_r($data); die();
	...
	public function getInformationname($information_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "information i LEFT JOIN " . DB_PREFIX . "information_description id ON (i.information_id = id.information_id) WHERE i.information_id = '" . (int)$information_id . "'");
	...
	/* 17 08 2018*/
	public function getCategoriesname($category_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "job_category jc LEFT JOIN " . DB_PREFIX . "job_category_description jcd ON (jc.category_id = jcd.category_id) WHERE jc.category_id = '" . (int)$category_id . "'");

		return $query->row;
	}

	public function getjobname($data) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "job_description WHERE job_id<>0");

		return $query->rows;
	}

	public function getjoblistname($job_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "job j LEFT JOIN " . DB_PREFIX . "job_description jd ON (j.job_id = jd.job_id) WHERE j.job_id = '" . (int)$job_id . "'");
