...
class ModelJobMail extends Model {
	public function addMail($data) {
		...
		companytype='".$this->db->escape($data['companytype'])."',
	...
	public function getMailLanguage($mail_id) {
		$mail_data = array();
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX ."mail_language WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' and mail_id = '" . (int)$mail_id . "'");
