...
class ControllerExtensionModuleFeatured extends Controller {
	...
	public function index() {
		...
		if (!empty($this->request->post['product'])) {
			$jobdetailinfo = $this->request->post['product'];
		} elseif (!empty($module_info['product'])) {
			$jobdetailinfo = $module_info['product'];
		} else {
			$jobdetailinfo = array();
		}

		foreach ($jobdetailinfo as $job_id) {
			$product_info = $this->model_job_Joblist->getJoblist($job_id);

			if ($product_info) {
				$data['jobdetailinfo'][] = array(
					'job_id' => $product_info['job_id'],
