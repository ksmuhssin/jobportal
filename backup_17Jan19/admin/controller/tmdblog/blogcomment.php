...
class ControllerTmdBlogBlogComment extends Controller {
	...
	if (isset($this->request->get['filter_name'])) {
		...
		if (is_file(DIR_IMAGE . $result['commentimage'])) {
			...
			if(isset($result['employ_id'])){
				$employ_info = $this->model_job_employ->getEmploy($result['employ_id']);
			}
			if(isset($employ_info['fullname'])){
				$name = $employ_info['fullname'];
			} else {
				$name = '';
			}
	...
	protected function getForm() {
		...
		$data['lable_customername'] = $this->language->get('lable_customername');
		$data['lable_tmdblog'] 		= $this->language->get('lable_tmdblog');
		...
		$data['lable_comment'] 		= $this->language->get('lable_comment');
		...
		if (isset($this->request->post['employ_id'])) {
			$data['employ_id'] = $this->request->post['employ_id'];
		} elseif (!empty($blog_info)) {
			$data['employ_id'] = $blog_info['employ_id'];
		} else {
			$data['employ_id'] = '';
		}
		...
		$employ_info = $this->model_job_employ->getEmploys($data);
	
		foreach ($employ_info as $info){
			$data['employers'][] = array(
			'employ_id' => $info['employ_id'],
