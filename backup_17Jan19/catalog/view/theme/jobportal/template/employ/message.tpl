      ...
      <h1><?php echo $heading_title; ?></h1>
		<form method="post" enctype="multipart/form-data" class="form-horizontal">
			...
			<div class="form-group required">
				...
				<div class="col-sm-10">
					<textarea name="message" placeholder="<?php echo $entry_description; ?>" id="description" class="form-control summernote"></textarea>
					<div id="val-d"></div>
				</div>
			</div>
			<div class="buttons">
...
<script>
	$('#enquerybutton').click(function(){
		...
		$("#val-d").html("message must be of 10 characters");
	     ...
	     $.ajax({
				url: 'index.php?route=employ/message/emailmessagesend',
				type: 'post',
				data: $('input[name=\'email\'],input[name=\'employ_id\'],textarea[name=\'message\']'),
				dataType: 'json',
				beforeSend: function() {
				},
				success: function(json) {
					if (json['success']) {
						...
						$("#email").val('');
						$("#description").val('');
						window.location='index.php?route=employ/message/view'
					}
				}
			});
		}
