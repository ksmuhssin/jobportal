...
class ModelTmdblogBlog extends Model {
	...
	$query = $this->db->query("SELECT *,b.image as commimage FROM ". DB_PREFIX ."blogcomment as b left join " . DB_PREFIX . "blog as a on(b.blog_id = a.blog_id)  left join " . DB_PREFIX . "user as u on(a.user_id = u.user_id) WHERE a.blog_id = '". $blog_id. "' AND b.status=1  ORDER BY b.date_added   DESC");	
	///$query = $this->db->query("SELECT *,b.image as commimage FROM ". DB_PREFIX ."blogcomment as b left join " . DB_PREFIX . "blog as a on(b.blog_id = a.blog_id)  left join " . DB_PREFIX . "employ as u on(b.employ_id = u.employ_id) WHERE a.blog_id = '". $blog_id. "'  ORDER BY b.date_added   DESC");	
	...
	$this->db->query("INSERT INTO " . DB_PREFIX . "blogcomment set comment = '". $this->db->escape($data['comment']). "', blog_id = '". (int)($data['blog_id']). "', image = '". ($data['image']). "', employ_id = '". $this->employ->getId(). "', status = '1', date_added = NOW()");		
	}
	
	public function Updatecomment($blog_id) {
	
	$query = $this->db->query("SELECT COUNT(*) as total FROM ". DB_PREFIX ."blogcomment  WHERE blog_id = '". $blog_id. "' and status=1");	
	...
	$sql = "SELECT b.comment, a.blog_id, a.viewed,a.date_added,b.customer_id, c.firstname, c.lastname FROM ". DB_PREFIX ."blogcomment b left Join ".DB_PREFIX."blog as a on(b.blog_id = a.blog_id) LEFT JOIN ". DB_PREFIX ."customer as c ON(b.customer_id = c.customer_id) WHERE a.date_added <= NOW() ORDER BY a.viewed DESC, a.date_added DESC LIMIT " . (int)$limit;	
	$query = $this->db->query($sql);	
	return $query->rows;
