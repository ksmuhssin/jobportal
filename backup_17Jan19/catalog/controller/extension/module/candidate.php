...
class ControllerExtensionModuleCandidate extends Controller {
	public function index($setting) {
		$this->language->load('extension/module/candidate');
        ...
        $employs_infos = $this->model_job_employ->getEmploys($filter_data);  

        foreach ($employs_infos as $employs_info){
	        if(!empty($employs_info['image'])){
		...
		return $this->load->view('extension/module/candidate', $data);
	}
}
?>
